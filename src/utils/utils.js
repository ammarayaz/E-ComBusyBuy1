...
// Function to get new items from data.js
const getNewItemsFromDataJs = async () => {
  const productSnapshot = await getDocs(collection(db, "products"));
  const existingProducts = productSnapshot.docs.map((doc) => doc.data()); // Filter out items that are already in the Firestore collection

  const newItems = data.filter(
    (item) => !existingProducts.some((product) => product.id === item.id)
  );
